...
bindkey '\E' backward-kill-line                   # ESC
bindkey '^Y' emacs-forward-word                   # ctrl + E
...
configure_prompt() {
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
            ...
            ;;
        oneline)
            PROMPT=$'%B%F{%(#.red.blue)}summer'$prompt_symbol$' %{%F{blue}%}%D{%T} %D{%m/%f} %B%F{reset}%b%F{%(#.blue.green)}[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}]%(#.#.$) '
            RPROMPT=
            ;;
        backtrack)
...
if [ "$color_prompt" = yes ]; then
    ...
    if [ -f ~/.oh-my-zsh/custom/plugins/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh ]; then
        ...
        ZSH_HIGHLIGHT_STYLES[autodirectory]=fg=default,underline
